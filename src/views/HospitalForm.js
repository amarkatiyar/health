import React from 'react'
import { Card,
        CardHeader 
        } from 'shards-react'

export default class HospitalForm extends React.Component{
    constructor(props) {
        super(props);
        this.state = { 
          open: false,
          handleShowHosF :false
      };
  }
  handleShowHosF=() => {
    this.props.handleHideHosF();
}

render(){
    const { showHosF } = this.state;

return(
<div>
<Card style={{height:"500px",width:"600px"}}>
        <CardHeader className="p-3 " style={{background:"#A9A9A9"}}>
        <div className="row" >
        
        <div onClick ={()=>this.handleShowHosF()} className="col-sm-1" > <i className="fas fa-arrow-left" style={{cursor:"pointer"}}></i></div>
            <div className="col-sm-1"style={{cursor:"pointer"}}> %</div>
            <div className="col-sm-10"> Service Charges/ Tax</div>
        </div>
        </CardHeader>
        <div className=" p-3" style={{background:"hsl(0, 0%, 94%)"}}>
            <div className="row">
                <div className="col-sm-9"><i class="fas fa-search" style={{cursor:"pointer"}}></i></div>
                <div className="col-sm-1"><span className=" hover icon-md "></span></div>
                <div className="col-sm-1"><span className=" hover icon-md "></span></div>
                <div className="col-sm-1"><span className=" hover icon-md fas fa-plus"style={{cursor:"pointer"}}></span></div>
            </div>
        </div>
            <div className="row mt-4">
                <div className="col-lg-2"></div>
                <div className="col-lg-8 bg-success text-left"style={{height:"40px"}}>
                <small><b>Add your Service Charges/Tax code here and use them quickly</b></small>
                </div>
            </div>
</Card>
</div>
        );
    }
}

